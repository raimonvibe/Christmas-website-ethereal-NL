...
(function($) {
	...
	// Snowflake Animation Functions.
		function createSnowflake(button) {
			...
			snowflake.style.left = (Math.random() * button.offsetWidth) + 'px';
			...
			setTimeout(function() {
				snowflake.style.top = (button.offsetHeight + 20) + 'px';
				snowflake.style.left = (Math.random() * button.offsetWidth) + 'px';
			}, 50);
	...
	// Add snowflake effect to buttons.
		$window.on('load', function() {
			$('.button').each(function() {
				...
				// Make button position relative if not already positioned
